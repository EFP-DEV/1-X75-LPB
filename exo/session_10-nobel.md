...
* `Étape 3 - requête api`
* `Étape 4 - affichage des prix nobel`
...
L’API à utiliser est l’API officielle du prix Nobel. L’endpoint `https://api.nobelprize.org/2.1/nobelPrizes` renvoie les données des prix Nobel au format JSON.

---

## Objectif général

Vous allez construire une interface qui permet, au clic sur un bouton, de charger une liste de prix Nobel depuis une API distante et de l’afficher dans la page.
...
* l’année du prix ;
* la catégorie du prix.

Vous pouvez enrichir l’affichage si vous le souhaitez, mais ce n’est pas obligatoire.
...
* lancez une requête avec `fetch()` vers l’API Nobel ;
...
* insérez dans chaque élément les informations minimales demandées ;
* ajoutez chaque élément dans la liste HTML prévue.

La liste doit être construite uniquement via le DOM.

Commit attendu :
`Étape 4 - affichage dynamique des prix`
